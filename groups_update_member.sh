...
members="{
           ...
           file_name="update_members.json"
           echo "$updated_members" > "$file_name"


           ${PROVENANCE_DEV_DIR}/build/provenanced tx group update-group-members "${alice}" 9 ./update_members.json  --home ${PROVENANCE_DEV_DIR}/build/node0 \
                                     --chain-id chain-local \
                                                              --gas auto --gas-adjustment 1.25 --gas-prices 1905nhash \
                                                              --testnet \
                                                              --broadcast-mode block \
